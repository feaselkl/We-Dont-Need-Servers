...
-- Here is an example of writing a single month to a single folder.
...
BEGIN
    ...
    SET @sql = N'CREATE EXTERNAL TABLE [dbo].[ArabilityScore_{Year}{Month}] WITH (
        LOCATION = ''ArabilityScore/{Year}/{Month}/'',
        DATA_SOURCE = [MarsFarmingCurated],
        FILE_FORMAT = [ParquetFileFormat]
) AS
SELECT *
