...
BEGIN
    ...
    SET @sql = N'CREATE EXTERNAL TABLE [dbo].[Temperature_{Year}{Month}] WITH (
        LOCATION = ''Temperature/{Year}/{Month}/'',
...
FROM OPENROWSET
(
    BULK ''abfss://synapse@bdadatalake.dfs.core.windows.net/marsfarming_raw/Temperature/*.txt'',
    FORMAT = ''CSV'',
    PARSER_VERSION = ''2.0'',
    FIELDTERMINATOR = '';'',
    FIRSTROW = 1
